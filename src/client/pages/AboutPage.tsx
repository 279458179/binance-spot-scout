...
export function AboutPage(): ReactNode {
  return (
    <section className="mx-auto w-full max-w-3xl px-0 py-0">
      <motion.header variants={fadeInUp} initial="hidden" animate="visible">
        <h1 className="text-[2rem] font-semibold text-[var(--text-primary)]">策略说明</h1>
        ...
        <table className="mt-2 w-full text-sm">
          <tbody>
            {SCORE_ITEMS.map((item) => (
              <tr key={item.key} className="border-b border-[var(--divider)] last:border-0">
                <th
                  scope="row"
                  className="py-3 text-left text-sm font-medium text-[var(--text-primary)]"
                >
                  {item.label}
                </th>
                <td className="tabular py-3 text-right text-sm text-[var(--text-primary)]">
                  {item.max} 分
                </td>
              </tr>
            ))}
      ...
      >
        {RULES.map((rule) => (
          <div key={rule.title} className="panel px-4 py-3">
            <p className="text-[15px] font-semibold text-[var(--text-primary)]">{rule.title}</p>
            <p className="mt-1.5 text-sm leading-6 text-[var(--text-secondary)]">{rule.body}</p>
          </div>
        ))}
      </motion.div>
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className="panel mt-5 border-[var(--border-soft)] px-5 py-5"
      >
